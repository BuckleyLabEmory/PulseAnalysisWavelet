...
if all(string(wtbl.potential_window_discard_reason) == 'All NaNs') | isempty(pl)
    dat_tbl = table();
...
%% MOROPHOLOGICAL QUANTIFICATION
    ...
    % systolic and diastolic auc
    binsdiff = diff(wtbl.bins_ln{w});
    bin = binsdiff(1);
    tbin_ln_pk = wtbl.tbin_ln{w};
    tbin_ln_pke = wtbl.tbin_ln{w};
    w_peak = cell2mat(wtbl.psf_loc_ln(w))*bin;
    tbin_ln_pk(tbin_ln_pk>w_peak) = [];
    %tbin_ln_pke(tbin_ln_pke<w_peak) = [];
    avg_bfi_ln_p = avg_bfi_ln-avg_bfi_ln(end);
    avg_bfi_ln_pk = avg_bfi_ln_p(1:numel(tbin_ln_pk));
    %avg_bfi_ln_pke = avg_bfi_ln_p(end-numel(tbin_ln_pke)+1:end);

    wtbl.f_auc_systolic(w) = {trapz(tbin_ln_pk,avg_bfi_ln_pk)};
    wtbl.f_auc_diastolic(w) = {cell2mat(wtbl.f_auc_subedf(w)) - cell2mat(wtbl.f_auc_systolic(w))};
    %wtbl.f_auc_diastolic_measured(w) = {trapz(tbin_ln_pke,avg_bfi_ln_pke)};
