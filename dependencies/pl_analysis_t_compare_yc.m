function dt = pl_analysis_t_compare(TimeAxis_DCS,ABP_or_Dbfit_wv,Reference_wave,Dbfit,numnans,varargin)
...
%%
    ...
    addRequired(p,'ABP_or_Dbfit_wv')
    addRequired(p,'Reference_wave')
    addRequired(p,'Dbfit')
    addRequired(p,'numnans')
    addOptional(p,'FileID',[])
    parse(p,TimeAxis_DCS,ABP_or_Dbfit_wv,Reference_wave,Dbfit,numnans,varargin{:});
    ...
    if strcmp(ABP_or_Dbfit_wv, 'ABP')
       [t_ABP, t_ABP_u, ~] = unique(Reference_wave(:,1));
       temp = Reference_wave(:,3);
       dT_all_ABP = diff(Reference_wave(:,1));
    elseif strcmp(ABP_or_Dbfit_wv, 'Dbfit_wv')
       [t_ABP, t_ABP_u, ~] = unique(Reference_wave(:,1));
       temp = Reference_wave(:,2);
       dT_all_ABP = diff(Reference_wave(:,1));
    else
        disp('Our Reference_wave need to be ABP or Dbfit_wv');
    end
    ...
    dT_DCS = mode(dT_all_DCS);
    if isduration(dT_DCS)
        dT_DCS = seconds(dT_DCS);
        f_DCS = 1/dT_DCS;
    else
        f_DCS = 1/dT_DCS;
    end
